...
CAPTURE_LAUNCH="${THESIS_TASK3_CAPTURE_LAUNCH:-${THESIS_WS}/launch/scenarios/task3_waypoint_capture.launch}"
CATKIN_WS="${THESIS_CATKIN_WS:-${HOME}/catkin_ws}"
CAPTURE_TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_FILE="${THESIS_TASK3_CAPTURE_FILE:-${THESIS_WS}/tasks/waypoint_sets/patrol_capture_${CAPTURE_TIMESTAMP}.yaml}"
...
resolve_map_path() {
  ...
  case "${raw_path}" in
    ...
    catkin_ws/*)
      ...
      ;;
    maps/*|config/*|launch/*|results/*|bags/*|logs/*|scripts/*|tasks/*)
      printf '%s\n' "${THESIS_WS}/${raw_path}"
      ;;
    *)
      printf '%s\n' "${raw_path}"
...
echo "Launching Task3 waypoint capture with active_map_id=${active_map_id}"
echo "Resolved map source: ${map_source}"
echo "Using map file: ${map_file}"
echo "Captured waypoints will be written to: ${OUTPUT_FILE}"

exec roslaunch "${CAPTURE_LAUNCH}" \
